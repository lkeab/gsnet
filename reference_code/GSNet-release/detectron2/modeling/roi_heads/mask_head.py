...
def mask_rcnn_loss(pred_mask_logits, instances):
    ...
    #print('gt mask shape:', gt_masks.to(dtype=torch.float32).shape)
    #print('bo gt mask shape:', gt_bo_masks.to(dtype=torch.float32).shape)
    #print('gt mask value:', gt_masks.to(dtype=torch.float32)[0,:,:])
    #print('bo gt mask value:', gt_bo_masks.to(dtype=torch.float32)[0,:,:])
    #indexs1 = torch.nonzero(torch.sum(gt_masks.to(dtype=torch.float32),(1,2)))
    #print('gt mask indexs:', indexs1.shape)
    #print('bo gt mask indexs:', indexs2.shape)

    mask_loss = F.binary_cross_entropy_with_logits(
        pred_mask_logits_gt, gt_masks.to(dtype=torch.float32), reduction="mean"
    )

    return mask_loss, boundary_loss, bo_mask_loss, bo_bound_loss
